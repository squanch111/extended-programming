...
## 2. [After Bootstrapping] Possion-simulated Death Trajectory VS Simulated Death Trajectory  # This plot can show the uncertainty
...
deconv <- function(t, deaths, n.rep=100, bs=FALSE, t0=NULL) {
  ...
  for (rep in 1:n.rep) {
    ...
    # Calculate the daily simulated death count dsi
    dsi <- tabulate(simulated_deaths, nbins=310)
    
    # add bootstrap
    if (bs){
      death_bs <- rpois(n, death) # create a n vector generated by Poisson random distribution
      filtered_values <- death_bs[death_bs >= 1 & death_bs <= 310] # Filter valid value
      ...
      death_bs <- filtered_values[1:n]  # The first n random numbers that meet the conditions are retained
      di <- tabulate(death_bs, nbins=310)  # Update actual death data
      if(rep == 1){
...
bootstrap_result <- deconv(data$julian, data$nhs, bs=TRUE, t0=result_optimized$t0)
